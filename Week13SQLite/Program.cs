...
using System.Data.SQLite; //Andmebaasi failiga ühenduse loomine
...
static void ReadData(SQLiteConnection myConnection) //Andmete lisamine
{
    Console.Clear();
...
static void RemoveCustomer(SQLiteConnection myConnection) //Andmete kustutamine
{
    ...
    Console.WriteLine($"{rowRemoved} was remowed fromthe table customer.");
...
{
    {
        ...
        command = myConnection.CreateCommand();
        command.
CommandText = $"SELECT customer.rowid, customer.firstName, customer.lastName, status.statustype " +
$"FROM customerStatus " +
